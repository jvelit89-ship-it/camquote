...
async function fixPhpProxy() {
    const client = new ftp.Client();
    try {
        console.log("📥 Conectando por FTP para arreglar proxy.php...");
        await client.access({
        ...
        if (curl_errno($ch)) {
            ...
            echo "Bad Gateway: Next.js server on port 3000 is unreachable. Error: " . curl_error($ch);
            curl_close($ch);
            exit;
        }
        ...
        console.log("✅ PHP Proxy arreglado.");
